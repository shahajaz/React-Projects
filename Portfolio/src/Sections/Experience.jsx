...
export default function Projects() {
  return (
    <section
      id="experience"
      className="min-h-fit md:min-h-[130vh] flex flex-col items-center pt-20 md:pt-24 pb-16 md:pb-40 relative text-white bg-[#2F4F4F] px-6 overflow-hidden scroll-mt-20"
    >
      {/* Background Glow */}
      ...
      >
        ...
        bg-black/80 backdrop-blur-md relative inline-block mx-auto whitespace-nowrap overflow-x-auto">
      ...
      {/* Timeline */}
      <motion.div
        className="relative max-w-6xl mx-auto py-3"
        initial={{ opacity: 0, scale: 0.9 }}
        whileInView={{ opacity: 1, scale: 1 }}
        viewport={{ once: false, amount: 0.3 }}
        transition={{ duration: 0.6, ease: "easeOut" }}
      >
        {/* Center Line */}
        <div className="hidden md:block absolute top-8 bottom-20 left-1/2 -translate-x-1/2 w-[3px] rounded-xl bg-gradient-to-b from-[#ff00af] to-[#ffcc00]"></div>
        ...
        {experiences.map((exp, index) => (
          <motion.div
            ...
            viewport={{ once: false, amount: 0.3 }}
            transition={{ duration: 0.6, ease: "easeOut" }}
            className={`relative mb-10 md:mb-20 flex flex-col md:flex-row ${
              index % 2 === 0
          ...
          >
            {/* Card */}
            <div className="relative w-full md:w-[50%] p-[1px] rounded-xl bg-gradient-to-b from-[#ff00af] to-[#ffcc00]">
              
              {/* Inner Card */}
              <div className="bg-[#0d0d0d] rounded-2xl p-6 md:p-8 h-full">
                <h3 className="text-lg md:text-xl font-semibold text-[#ffcc00] leading-tight">
                  {exp.role}
                </h3>
                ...
                <div className="flex items-center gap-3 mt-3">
                  <img
                    src={exp.logo}
                    alt={exp.company}
                    className="w-9 h-9 md:w-10 md:h-10 object-contain rounded-md bg-white/10 p-1 border border-white/10"
                  />
                ...
                {/* Date */}
                <p className="text-xs text-gray-500 mt-1 ml-12 md:ml-14">
                  {exp.date} • {exp.location}
                </p>

                {/* Description */}
                <p className="mt-4 md:mt-5 text-gray-300 text-sm leading-relaxed">
                  {exp.description}
                </p>

                {/* Achievements */}
                <ul className="mt-4 md:mt-5 space-y-2 text-sm text-gray-300">
                ...
                <div className="flex flex-wrap gap-2 mt-4 md:mt-5">
                  {exp.tech.map((t, i) => (
                    <span
                      key={i}
                      className="px-2 md:px-3 py-1 text-xs bg-[#ff00af]/10 text-[#ffcc00] rounded-full border border-[#ff00af]/30"
                    >
                      {t}
                    </span>
                  ))}
